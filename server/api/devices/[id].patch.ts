...
export default defineEventHandler(async (event) => {
  ...
  if (body && "esDeRootRelPath" in body) {
    if (body.esDeRootRelPath === null || body.esDeRootRelPath === "") {
      delete dev.esDeRootRelPath;
    } else if (typeof body.esDeRootRelPath === "string") {
      const trimmed = body.esDeRootRelPath.trim();
      dev.esDeRootRelPath = trimmed ? normalizeRelPath(trimmed) : undefined;
      if (!dev.esDeRootRelPath) delete dev.esDeRootRelPath;
    } else {
      throw createError({
        statusCode: 400,
  ...
  if (body && "artMaxEdgePx" in body) {
    if (body.artMaxEdgePx === null || body.artMaxEdgePx === 0) {
      delete dev.artMaxEdgePx;
    } else if (
      typeof body.artMaxEdgePx === "number" &&
      Number.isFinite(body.artMaxEdgePx) &&
      body.artMaxEdgePx > 0
    ) {
      dev.artMaxEdgePx = Math.floor(body.artMaxEdgePx);
    } else {
      throw createError({
        statusCode: 400,
